    ...
    public class UserBusiness
    {
        ...
        public async Task<UserDto> CreateUserAsync(UserDto userDto)
        {
            try
            {
                ...
                // Considerar añadir fechas si existen en la entidad User (CreateDate, etc.)
                // user.CreateDate = DateTime.UtcNow;

                var userCreado = await _userData.CreateAsync(user);
                return MapToDTO(userCreado);
            }
            catch (DbUpdateException dbEx) // Capturar posible violación de UNIQUE constraint (Username, Email)
            {
                _logger.LogError(dbEx, "Error de base de datos al crear usuario. Posible duplicado de Username/Email.");
                // Analizar InnerException para determinar la causa exacta si es necesario
